...
inline void IntervalTree<T>::clear() {
	...
	std::vector<uint16_t>().swap(m_nodesToRemove);
	m_root = -1;
}
...
template <typename T>
int16_t IntervalTree<T>::getInterval(size_t index) const {
	int32_t interval = m_root;
	while (true) {
...
inline void IntervalTree<T>::rotateParentLeft(int index, Node* grandParent) {
	...
	int16_t parentIndex = node.parent;
