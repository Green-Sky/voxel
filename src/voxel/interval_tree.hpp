...
#ifndef vorb_interval_tree_hpp
#define vorb_interval_tree_hpp
...
#define START_MASK 0x7FFFu
		...
		class LNode {
			public:
				LNode() {}
				LNode(uint16_t Start, uint16_t Length, T Data) : start(Start), length(Length), data(Data) {}
				void set(uint16_t Start, uint16_t Length, T Data) {
					start = Start;
					length = Length;
					data = Data;
				}
		...
		class Node {
			public:
				...
				Node(T Data, uint16_t start, uint16_t Length) : length(Length), left(-1), right(-1), parent(-1), m_start(start | COLOR_BIT), data(Data)  {}

				inline void incrementStart() { ++m_start; }
				inline void decrementStart() { --m_start; }
				inline uint16_t getStart() const { return m_start & START_MASK; }
				inline void setStart(uint16_t Start) { m_start = (m_start & COLOR_BIT) | Start; }
				...
				int16_t parent;
			private:
				uint16_t m_start; //also stores color
			public:
				T data;
		};
		...
		int16_t getInterval(size_t index) const;
	...
	private:
		...
		class NodeToAdd {
		public:
			NodeToAdd(int16_t Parent, uint16_t Length, T Data) : start(Parent), length(Length), data(Data) {}
			int16_t start;
			uint16_t length;
			T data;
		};
		...
		std::vector <uint16_t> m_nodesToRemove;
};
